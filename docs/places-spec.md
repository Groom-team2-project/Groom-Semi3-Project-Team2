...
Places 기능에서는 카카오 지도 Web API와 카카오 로컬 REST API를 함께 사용한다. 두 API의 역할과 실행 위치는 서로 다르다.

### 3.1 카카오 지도 Web API

프론트엔드에서 사용하며 다음 기능을 담당한다.
...
지도 Web API가 장소 정보를 자동으로 검색하는 것은 아니다. 백엔드가 반환한 장소의 `latitude`, `longitude`를 프론트엔드가 전달받아 지도에 마커를 생성한다.
...
백엔드에서 사용하며 다음 기능을 담당한다.

- 카테고리로 장소 검색
- 장소명 또는 일반 키워드로 장소 검색
- 중심 좌표와 반경을 기준으로 장소 검색
- 현재 지도 화면의 사각형 영역을 기준으로 장소 검색
- 장소 목록과 페이징 정보 제공

카카오 로컬 REST API 호출에는 REST API 키를 사용한다.

```http
Authorization: KakaoAK {REST_API_KEY}
```

REST API 키는 백엔드 환경변수나 비밀 설정으로 관리하며 프론트엔드, API 응답, 로그에 노출하지 않는다.

카카오 REST API의 좌표 이름은 다음과 같다.
...
지도 영역 검색에 사용하는 `rect`의 순서는 다음과 같다.

```text
남서쪽 경도, 남서쪽 위도, 북동쪽 경도, 북동쪽 위도
```

### 3.3 카카오에서 받을 수 있는 장소 정보

카카오 키워드 장소 검색과 카테고리 장소 검색을 통해 다음 정보를 받을 수 있다.
...
- 위도와 경도
- 카카오맵 장소 상세 페이지 URL
- 중심 좌표를 사용한 경우 해당 좌표로부터의 거리
- 전체 결과 수와 다음 페이지 여부 등의 페이징 정보

서비스에서는 카카오 원본 JSON을 그대로 프론트엔드에 전달하지 않고, 9절에 정의된 `PlaceSearchItem` 형태로 변환한다.

## 4. 프로젝트 구조와 요청 흐름

카카오 로컬 REST API와 지도 Web API가 서로 직접 연결되는 구조가 아니다. 백엔드가 로컬 REST API에서 장소 데이터를 받은 뒤 프론트엔드에 전달하고, 프론트엔드가 해당 데이터를 지도 Web API와 함께 사용한다.
...
사용자
 ...
 └─ 지도 이동
          ...
          ▼
프론트엔드
 ├─ 카카오 지도 Web API로 지도 표시
 ├─ 현재 지도 영역의 위도·경도 확인
 └─ 카테고리·검색어·지도 영역을 백엔드에 전달
          │
          ▼
우리 백엔드 Places API
 ├─ 검색 조건과 좌표 검증
 ├─ 서비스 카테고리를 카카오 그룹 코드로 변환
 └─ 카카오 로컬 REST API 호출
          │
          ▼
카카오 로컬 REST API
...
우리 백엔드 Places API
 ├─ 필요한 장소 정보만 추출
 ├─ 카카오 필드명을 서비스 필드명으로 변환
 └─ PlaceSearchItem 목록 반환
          │
          ▼
프론트엔드
 ├─ 장소 목록 표시
...
1. 사용자가 프론트엔드에서 음식점 버튼을 선택한다.
2. 프론트엔드는 카테고리 `RESTAURANT`와 현재 지도 영역을 백엔드에 전달한다.
3. 백엔드는 `RESTAURANT`를 카카오 그룹 코드 `FD6`으로 변환한다.
4. 백엔드는 카카오 카테고리 장소 검색 API에 `FD6`과 지도 영역 `rect`를 전달한다.
5. 카카오는 해당 지도 영역 안의 음식점 목록을 JSON으로 반환한다.
6. 백엔드는 JSON에서 필요한 정보만 `PlaceSearchItem`으로 변환한다.
7. 프론트엔드는 응답을 장소 목록과 지도 마커에 함께 사용한다.
...
4. 카카오가 검색어에 해당하는 장소 목록을 반환한다.
5. 백엔드는 결과를 `PlaceSearchItem`으로 변환한다.
6. 프론트엔드는 결과 목록과 마커를 표시한다.

카테고리가 선택된 상태에서 검색어를 입력하면 키워드와 카테고리 그룹 코드를 함께 전달하여 검색 범위를 제한할 수 있다.

### 4.3 지도 이동 후 재검색 흐름

1. 사용자가 지도를 이동하거나 확대·축소한다.
2. 프론트엔드는 지도 이동이 끝난 `idle` 이벤트를 감지한다.
3. `map.getBounds()`로 현재 지도 영역을 얻는다.
4. `getSouthWest()`와 `getNorthEast()`로 남서쪽·북동쪽 좌표를 얻는다.
5. 기존 카테고리 또는 검색어와 새로운 지도 영역을 백엔드에 전달한다.
6. 백엔드는 새로운 `rect`로 카카오 로컬 REST API를 다시 호출한다.
7. 프론트엔드는 기존 목록과 마커를 새로운 검색 결과로 갱신한다.

## 5. 공식 문서

- [카카오 로컬 REST API 개발 가이드](https://developers.kakao.com/docs/ko/local/dev-guide): 키워드·카테고리 장소 검색, 요청 파라미터, 응답 JSON 확인
- [카카오 지도 Web API 문서](https://apis.map.kakao.com/web/documentation/): 지도 생성·조작, 좌표 객체, 지도 영역, 이벤트, 마커 사용 방법 확인
- [카카오 지도 정보 얻어오기 예제](https://apis.map.kakao.com/web/sample/mapInfo/): `getBounds()`, `getSouthWest()`, `getNorthEast()` 사용 예제
- [카카오 REST API 시작하기](https://developers.kakao.com/docs/ko/rest-api/getting-started): REST API 키를 사용한 서버 요청과 공통 인증 방법 확인
